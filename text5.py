...
def lottemart(sdt): # API lottemart CALL
    ...
    try:
        if response ['error'] == '':
            print("[@truongdeptrai7] ~ LOTTEMART SPAM THÀNH CÔNG")
        else:
            print("[@truongdeptrai7] ~ LOTTEMART SPAM THẤT BẠI")
    except:
        print("[@truongdeptrai7] ~ LOTTEMART SPAM THẤT BẠI")
def robotq(phone):
    cookies = {
    ...
    try:
        if response ['error'] == '':
            print("[@truongdeptrai7] ~ VIETLOAN SPAM THÀNH CÔNG")
        else:
            print("[@truongdeptrai7] ~ Đã Spam Thành Công")
    except:
        print("[@truongdeptrai7] ~ Đã Spam Thành Công")
def fb(phone):
    cookies = {
...
def kingz(phone):
    ...
    try:
        if response ['error'] == '':
            print("[@truongdeptrai7] ~  SPAM THÀNH CÔNG")
        else:
            print("[@truongdeptrai7] ~  SPAM THẤT BẠI")
    except:
        print("[@truongdeptrai7] ~  SPAM THẤT BẠI")
def med(phone):
    headers = {
...
functions = [  
      vietloana,vietloanf,robot,robota,robots,vietloanaa,robotd,robotf,robotg,roboth,robotj,robotk,robotl,robotq ,robotz,robotx,robotc,robotv,robotb,robotn,robotm,chotot, tv360, robot, fb, mocha, dvcd, myvt, phar, dkimu, fptshop, meta, blu,
...
def run(phone, i):
    ...
    try:
        print(f"Spam thành công lần {i}")  # This should now work correctly
    except UnicodeEncodeError:
        print(f"Spam thành công lần {i}".encode('utf-8', errors='ignore').decode('utf-8'))
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python text.py <phone_number> <count>")
        sys.exit(1)

    phone_number = sys.argv[1]  # Fetch phone number from command-line argument
    count = int(sys.argv[2])  # Fetch count from command-line argument

    for i in range(1, count + 1):
        run(phone_number, i)  # Call the run function for each count
         # Wait a seh count from command-line argument
